...
public final class Event: EventDescriptor {
    ...
    public var borderColor = SystemColors.systemBlue.withAlphaComponent(0.6)
    ...
    public var descriptionFont: UIFont = UIFont.systemFont(ofSize: 12)
    public var descriptionColor: UIColor = SystemColors.secondaryLabel
    ...
    private func applyStandardColors() {
        ...
        borderColor = dynamicStandardBorderColor()
    }
    ...
    /// Dynamic color that changes depending on the user interface style (dark / light)
    private func dynamicStandardBorderColor() -> UIColor {
        let light = borderColorForLightTheme(baseColor: color)
        let dark = borderColorForDarkTheme(baseColor: color)
        return dynamicColor(light: light, dark: dark)
    }
    ...
    private func borderColorForLightTheme(baseColor: UIColor) -> UIColor {
        baseColor.withAlphaComponent(0.45)
    }
    
    private func borderColorForDarkTheme(baseColor: UIColor) -> UIColor {
        var h: CGFloat = 0, s: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
        color.getHue(&h, saturation: &s, brightness: &b, alpha: &a)
        return UIColor(hue: h, saturation: s, brightness: b * 0.6, alpha: a * 0.6)
    }
