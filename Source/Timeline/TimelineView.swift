...
//            if hour == hourToRemoveIndex { continue }
    ...
    private func layoutEvents() {
        ...
        for (idx, attributes) in regularLayoutAttributes.enumerated() {
            ...
            var x: CGFloat
            if UIView.userInterfaceLayoutDirection(for: semanticContentAttribute) == .rightToLeft {
                x = bounds.width - attributes.frame.minX - attributes.frame.width + 8
            } else {
                x = attributes.frame.minX + 8
            }
    ...
    private func recalculateEventLayout() {
        ...
        let widthMultiplier: CGFloat = style.presentation == .oneDay ? 1 : 1/3
        ...
        for overlappingEvents in groupsOfEvents {
            ...
            for (index, event) in overlappingEvents.enumerated() {
                ...
                let xOffset: CGFloat = dateToXOffset(event.descriptor.startDate) * calendarWidth * widthMultiplier
                let x = (style.leadingInset + floatIndex / totalCount * (calendarWidth * widthMultiplier)) + xOffset
                let equalWidth = (calendarWidth / totalCount) * widthMultiplier
                event.frame = CGRect(x: x, y: startY, width: equalWidth - style.eventGap, height: endY - startY)
            }
        }
    }
