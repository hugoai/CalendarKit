...
open class EventView: UIView {
    ...
    public var contentHeight: CGFloat {
        return textView.frame.height + descriptionTextView.frame.height
    }
    ...
    public func updateWithDescriptor(event: EventDescriptor) {
        ...
        descriptionTextView.textColor = event.descriptionColor
        descriptionTextView.font = event.descriptionFont
        if let lineBreakMode = event.lineBreakMode {
            textView.textContainer.lineBreakMode = lineBreakMode
            descriptionTextView.textContainer.lineBreakMode = lineBreakMode
        }
        descriptor = event
    ...
    private func constrainTexts() {
        let minX: CGFloat = UIView.userInterfaceLayoutDirection(for: semanticContentAttribute) == .rightToLeft ? 0 : 3
        textView.translatesAutoresizingMaskIntoConstraints = false
        descriptionTextView.translatesAutoresizingMaskIntoConstraints = false
        let constraints = [
            textView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: minX),
            textView.topAnchor.constraint(equalTo: topAnchor, constant: (frame.minY < 0 ? (frame.minY * -1) : 0)),
            textView.trailingAnchor.constraint(equalTo: trailingAnchor),
            descriptionTextView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: minX),
            descriptionTextView.topAnchor.constraint(equalTo: textView.bottomAnchor, constant: -10),
            descriptionTextView.trailingAnchor.constraint(equalTo: trailingAnchor),
            descriptionTextView.bottomAnchor.constraint(lessThanOrEqualTo: bottomAnchor)
        ]
